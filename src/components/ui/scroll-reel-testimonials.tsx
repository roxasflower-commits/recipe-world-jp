...
const QUOTE_CLASSES =
  "m-0 text-lg font-medium leading-[1.3] tracking-[-0.02em] text-primary sm:text-[22px]";
const AUTHOR_CLASSES =
  "m-0 text-sm font-medium leading-[1.3] text-muted";
...
function Cell() {
  return (
    <div
      aria-hidden="true"
      className="shrink-0 rounded-xl border border-warm-border bg-gradient-to-b from-cream to-white blur-[1px] shadow-[0_1px_2px_rgba(0,0,0,0.05),inset_0_2px_0_rgba(255,255,255,1)]"
...
function Featured({ src, alt }: { src: string; alt?: string }) {
  return (
    <div
      className="relative shrink-0 overflow-hidden rounded-xl bg-cream"
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(220.99deg, rgba(45,90,61,0) 32%, rgb(45,90,61) 41%, rgb(100,140,113) 47%, rgba(45,90,61,0.57) 54%, rgba(45,90,61,0) 65%)",
        }}
      />
    </div>
  );
...
export function ScrollReelTestimonials({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className={cn(
        "relative flex w-full max-w-[1060px] flex-col items-stretch gap-2.5 overflow-hidden rounded-xl border border-warm-border bg-cream shadow-[inset_0_2px_0_rgba(255,255,255,1)] outline-none focus-visible:ring-2 focus-visible:ring-accent md:min-h-[320px] md:flex-row",
        className
      )}
    >
      {/* Reel */}
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between self-stretch px-5 py-7 md:py-10">
        ...
        <div className="mt-6 flex items-center gap-1.5 md:mt-0">
          <button
            ...
            disabled={index === 0}
            aria-label="前のシェフ"
            className="grid h-6 w-6 cursor-pointer place-items-center rounded-full border border-primary/15 bg-transparent p-0 text-primary transition-[opacity,transform] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] hover:enabled:scale-[1.08] active:enabled:scale-[0.94] disabled:cursor-default disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
          >
